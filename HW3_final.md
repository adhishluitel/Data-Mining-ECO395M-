...
In this exercise, we explore the data of followers of the Twitter
account of ‘NutrientH20’, and come up with some interesting insights
about the audience by identifying market segments.

### Data cleaning

We start with cleaning our data set which is comprised of 7,882
observations and 36 variables. First, we deleted all the users whose
tweet fell into ‘spam’ category at least once. It is not likely that
someone tweets a spam message unless (s)he’s a bot. And we got rid of
‘spam’ variable out of our data set.  
Then we also deleted ‘chatter’ and ‘uncategorized’ variables because the
company can hardly get useful information from these categories.  
Finally, ‘adult’ category can be interpreted as spam but sometimes it
can become a hobby, too. Considering that normal people don’t tweet much
porn in public, we deleted users that have more than 20% of their total
posts in pornography, regarding them as bots.  
...
of these means could help us finding our optimal K.

![](HW3_final_files/figure-markdown_strict/3.4.3-1.png)

It is difficult to find the optimal K by using mathematics, so we’ll use
our intuition instead. We’ll suppose that the company divides market
segments into three parts - one to focus on, one to maintain, one to
give up. We’ll see what happens when K=3, if it doesn’t seem to be
reasonable, we’ll keep going on K=4,5, etc.
...
using K=4 seems to be reasonable, so we’ll keep doing our analysis when
K=4.
